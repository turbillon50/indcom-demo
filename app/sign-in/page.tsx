      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column',
        ...
        <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }}
          ...
          <motion.button whileTap={{ scale:.97 }} onClick={handleLogin}
            disabled={loading}
            style={{ width:'100%', height:54, background:'var(--violet)',
              color:'white', fontWeight:800, fontSize:15, borderRadius:16,
              border:'none', cursor:'pointer', display:'flex', alignItems:'center',
              justifyContent:'center', gap:8, opacity:loading?.7:1 }}>
            {loading
              ? 'Verificando...'
              : <><span dangerouslySetInnerHTML={{ __html: SVG['check']??'' }}
                  style={{ display:'flex' }} />Ingresar</>}
          </motion.button>
